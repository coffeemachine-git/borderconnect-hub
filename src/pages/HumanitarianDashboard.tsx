...
import { Stethoscope, Home, UtensilsCrossed, Baby, Users } from "lucide-react";
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
          ...
          {[
            ...
            <Card key={s.label}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div><div className="text-2xl font-bold">{s.count}</div><div className="text-xs text-muted-foreground">{s.label}</div></div>
              </CardContent>
            </Card>
          ))}
        </div>
              ...
              <div className="space-y-2">
                {needsMedical.filter((t) => t.medicalPriority === "critical").map((t) => (
                  <div key={t.id} className="flex items-center justify-between p-2 rounded bg-card">
                    ...
                    <div className="flex gap-1">{t.aidNeeds.filter((n) => n !== "none").map((n) => <AidBadge key={n} need={n} />)}</div>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        {/* Aid list */}
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Travelers Requiring Assistance</CardTitle></CardHeader>
                ...
                {aidTravelers.map((t) => (
                  <tr key={t.id} className={`border-b last:border-0 hover:bg-muted/50 ${t.unaccompaniedMinor ? "bg-primary/5" : ""}`}>
                    ...
                    <td className="py-3 pr-4">
                      <div className="flex gap-1 flex-wrap">{t.aidNeeds.filter((n) => n !== "none").map((n) => <AidBadge key={n} need={n} />)}</div>
                    </td>
                    <td className="py-3 pr-4 text-xs hidden md:table-cell">{t.medicalPriority ?? "—"}</td>
                    <td className="py-3 hidden md:table-cell">{t.unaccompaniedMinor ? <span className="text-xs font-semibold text-primary">Yes</span> : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
